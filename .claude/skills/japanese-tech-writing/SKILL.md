...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定め、AI っぽい語彙・翻訳調・句読点の癖の点検と、textlint と suiko による機械 lint を含む。日本語で技術書の章、草稿、記事、解説文、メモ、論文本文を書くとき、推敲・リライトするとき、「AI っぽい」「AI 臭を抜いて」「自然な日本語に」と頼まれたときに使用する。
---
...
- 一つの段落には一つのトピックだけを置く。場面の進行（調査、報告、検証、評価）が複数入り交じった長い段落は、一歩ずつの段落に分割する。
...
- 山場で使いたい情報（数値、固有の事実）は、その手前の段落で先出ししない。
...
- 異なるものを「同じ」とまとめない。区別すべき対象（別々の決定、別々の原因、種類の違う問題）を一括りの言葉でくくらない。悪い例：相互依存する三つの未決事項を「同じ決定を別々に下していた」と書く。良い例：「どれも別々の決定であり、しかも互いに依存している」と分けて書く。
- 複数の要因がある事象を、単一の原因に還元しない。例が複数種類の問題を含むなら、それぞれを分け、どの概念がどれを説明するのかを対応づける。悪い例：契約の不在と情報隠蔽の失敗が重なった事故を、丸ごと「情報隠蔽の問題」と説明する。
...
- 「次節で扱う」と前方に逃がした論点は、本当にそこで答えることを確認する。答えない伏線を張らない。
...
- 例の節の中でも、**その節の問い・結論に関係しない**過剰な詳細だけを omit する。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名
...
- 予告、異変、種明かし、冒頭への回帰という筋や、逡巡してから断定する往復を、解説・報告・メモ・論文本文に持ち込まない。読者に事実の説明とは別の筋書きを追わせることになる。これらの装置は読み物に限り `../cognitive-rhythm-writing/SKILL.md` に従う。
...
日本語の散文をファイルに書き出して人に渡すとき（原稿、論文本文、共有する文書、`.tmp/*.md` の設計メモ）に行う。チャットの返答と handoff の `.tmp/progress.md` は対象外。kenq で推敲したユーザーの原稿も、直した後の版を対象にする。

- 書く前に `references/ai-vocabulary.md` と `references/ai-syntax.md` を読む。語の選択は書く時点で決まり、後からの置換では前後の文とのつながりが崩れる。
- 書き上げたら `~/.claude/skills/japanese-tech-writing/scripts/lint.sh <file>` を実行する。読み方と手順は `references/lint.md`。
- 指摘は候補であり命令ではない。指摘ごとに「直した」か「残す（理由）」をチャットの返答内の台帳に一行ずつ書く。理由のない「残す」は書かない。ユーザーが書いた文への指摘も同じ基準で判断する。
- 文脈から決められない指摘（著者の語の選択が意図的か、直すと意味が変わるか）は「要確認」として台帳に残し、収束してからまとめて聞く。5 件以下は AskUserQuestion の multiSelect で一回に聞く。それより多ければ `.tmp/ja-lint-ledger.md` に書き出し、ユーザーが各行の keep / change を書き換える。答えを反映したら lint をもう一度回す。書式は `references/lint.md`。
- 直したら再実行し、新しい指摘が出なくなるまで繰り返す。同じ指摘が二周続けて再発したら、言い換えを重ねず、段落の構造を書き直すか理由を付けて残す。
- ツールが無ければスクリプトが止まる。導入してから続け、目視の点検で代用しない。
- 指摘を全文へ一律に当てない。直す箇所を選び、既によい箇所と人が書いた痕跡は触らない。
