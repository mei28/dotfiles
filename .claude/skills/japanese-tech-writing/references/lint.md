...
文脈から決められない指摘は、直さずに「要確認」として残す。著者の語の選択が意図的に見えるとき、言い換えると意味が変わるとき、術語かどうかが本文だけでは決まらないときがこれにあたる。

```
- [要確認] no-ai-words「照合」L12: 認証の照合なら残す。比較の意味なら「比べる」に直す
```

要確認は収束してからまとめて聞く。指摘のたびに手を止めない。

- 5 件以下: AskUserQuestion の multiSelect で一回に聞く。選択肢は指摘ごとに「直す（案）」を示し、選ばれなかったものは残す
- それより多い: `.tmp/ja-lint-ledger.md` に次の形で書き出し、ユーザーが行末の `=> keep` を `=> change` か `=> change: <指示>` に書き換える。書き換えが戻ったら読み取って反映する

```
# ja-lint ledger: 原稿.md（2026-09-19）
- L12 no-ai-words「照合」: 認証の照合なら残す。比較なら「比べる」 => keep
- L40 antithesis_repetition: 4 箇所中 2 箇所を肯定文にする案。残り 2 箇所は論旨の核 => keep
```

答えを反映したら lint をもう一度回し、新しい指摘が出ないことを確かめる。台帳ファイルは作業が終わったら消す。

## 収束

出口は二つそろったときだけ。台帳の全指摘が「直した」「残す」「要確認」のいずれかに仕分け済みで、かつ直した結果を再実行しても新しい指摘が出ない。直したら必ず再実行する。要確認が残っていれば、その回答を反映した後にもう一度回す。
