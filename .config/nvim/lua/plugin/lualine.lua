...
function lualine_setup()
	...
	local function overleaf_status()
		local ok, overleaf = pcall(require, "overleaf")
		if not ok then
			return ""
		end

		local state = overleaf.get_state()
		if state == "connecting" or state == "reconnecting" or state == "authenticating" then
			return "🍃 ..."
		end
		...
		local name = overleaf.get_status().projectName or ""
		if #name > 8 then
			name = name:sub(1, 8) .. "…"
		end
		return "🍃 " .. name
