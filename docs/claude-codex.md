...
```
        ...
        └─────────┬──────────┘  Codex→Claude (mcp_servers.claude) └─────────┬──────────┘
                  └──────────────── .tmp/progress.md ─────────────────────┘
                          （上限フォールバック時の橋渡し。Antigravity も同じノートで再開可）
```
...
既知の別問題（herdrとは無関係、実機検証で判明）: このリポジトリの
`.codex/prompts/{resume,review,handoff}.md` はカスタム slash command として
運用する設計だが、Codex CLI 0.142.3 では `/resume` が組み込みの
セッション選択ピッカーと衝突し、`/handoff` は未認識になる。
このため `.claude/skills/codex-implement/SKILL.md` からは `/resume`・`/handoff`
の案内を削除済みで、計画は直接ペーストし、引き継ぎは Claude 側の `handoff`
skill で行う運用にしている。

未確認: 手元の Codex は 0.144.4 に上がっている。この版で解消したかは未検証。
解消していればカスタム prompt 運用に戻せるので、次に触るときに確認する。
...
| `GEMINI.md` | Antigravity CLI のプロジェクト指示（ルートで自動読込） |
