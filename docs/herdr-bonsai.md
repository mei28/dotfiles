...
マージの承認は片付けの承認ではない。
`herdr workspace close` は workspace の中のすべての pane を道連れにする。
`bonsai remove` はチェックアウトそのものを消す。
どちらも、利用者が直前のメッセージでそう言ったときだけ実行する。

作業が落ち着いたら、状態を報告してコマンドを提示するにとどめる。
実行するのは名指しされた対象だけだ。
あるブランチへの承認は、次のブランチへの承認ではない。
...
- マージしたら worktree と pane まで消えた
  → 片付けはマージとは別の承認事項である。マージの流れで続けて実行しない。
- 台帳には残してあると書いてあるのに worktree がない
  → 実行していない片付けを記録した、または記録せずに片付けた。`git worktree list` を正とみなして台帳を直す。
