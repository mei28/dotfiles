...
import { collectProviderUsage, findUsageWindow, readCachedProviderUsage } from "./providers.js";
import { resolveActiveModel } from "./opencode-client.js";
import { basename, formatPercent, formatTokenAmount, isRecord, toFiniteNumber, toNonEmptyString } from "./format.js";
import { loadStatuslineConfig } from "./statusline-config.js";
...
const gitDiffStatsCache = new Map();
export function invalidateGitDiffStatsCache() {
...
async function resolveTuiModel(api, sessionID) {
    const client = {
        session: {
...
function clientCallWithTimeout(run, ms) {
    return new Promise((resolve) => {
        ...
        try {
            run(controller.signal).then((value) => finish({ timedOut: false, value }), () => finish({ timedOut: controller.signal.aborted }));
        }
        catch {
...
function sessionTokenTotals(messages) {
...
function messageRecordedCost(message) {
    if (!isRecord(message) || message.role !== "assistant")
        return undefined;
...
function modelContextLimit(provider, modelID) {
    const model = modelInfo(provider, modelID);
    if (!model || !isRecord(model.limit))
        return undefined;
    return toFiniteNumber(model.limit.context);
}
function modelInfo(provider, modelID) {
    if (!provider || !modelID || !isRecord(provider.models))
        return undefined;
    const model = provider.models[modelID];
    return isRecord(model) ? model : undefined;
}
function modelCostRow(row, tierSize, experimental = false) {
...
function parseModelCostRows(model) {
    if (!model)
        return [];
...
function selectModelCostRow(rows, tokens) {
    if (rows.length === 0)
        return undefined;
...
function estimateMessageCost(input) {
    ...
    if (tokens.input + tokens.output + tokens.reasoning + tokens.cacheRead + tokens.cacheWrite <= 0)
        return undefined;
...
function partEndMs(part) {
    const time = isRecord(part.time) ? part.time : undefined;
    const state = isRecord(part.state) ? part.state : undefined;
    const stateTime = isRecord(state?.time) ? state.time : undefined;
    return timestampMs(time?.end) ?? timestampMs(stateTime?.end);
}
function latestAssistantMessages(messages) {
    const result = [];
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        const message = messages[index];
        if (isRecord(message) && message.role === "assistant")
...
function combinedIntervalDurationMs(intervals) {
...
function generationMetricsForMessage(api, message) {
    const messageID = toNonEmptyString(message?.id);
    if (!message || !messageID || !api.state.part)
        return undefined;
    ...
    const generationDurationMs = combinedIntervalDurationMs(generatedParts.flatMap((part) => {
...
function latestGenerationMetrics(api, messages) {
    let partial;
    for (const message of latestAssistantMessages(messages)) {
        const metrics = generationMetricsForMessage(api, message);
        if (!metrics)
            continue;
        if (metrics.ttftMs !== undefined && metrics.tokensPerSecond !== undefined)
            return metrics;
        partial ??= metrics;
    }
    return partial;
...
function generationMetricsText(metrics) {
    if (!metrics)
        return undefined;
    const parts = [];
    if (metrics.ttftMs !== undefined)
        parts.push(`ttft ${formatLatencyMs(metrics.ttftMs)}`);
    if (metrics.tokensPerSecond !== undefined)
        parts.push(`gen ${formatTokensPerSecond(metrics.tokensPerSecond)} tok/s`);
...
async function runGitNumstat(cwd, args) {
    try {
...
async function gitDiffStats(api) {
    ...
    if (cached && now - cached.loadedAt < GIT_DIFF_CACHE_TTL_MS)
        return cached.value;
    const value = loadGitDiffStats(cwd);
    gitDiffStatsCache.set(cwd, { loadedAt: now, value });
    return value;
}
function gitDiffStatsText(stats) {
...
function allowsBackgroundStatuslineRefresh(status) {
    return isInactiveSessionStatus(status) || status === "queued" || status === "pending";
}
function agentStatusText(status) {
    if (!status || ["queued", "pending"].includes(status))
        return undefined;
    return status;
}
async function getChildSessions(api, sessionID, timeoutMs = SESSION_CHILDREN_TIMEOUT_MS) {
    ...
    for (const args of attempts) {
        ...
        if (remainingMs <= 0)
            return [];
        try {
            const result = await clientCallWithTimeout((signal) => Promise.resolve(api.client?.session?.children?.(...args, { signal })), remainingMs);
            if (result.timedOut)
                return [];
...
function uniqueChildSessions(children, parentID) {
    ...
    for (const child of children) {
        ...
        seen.add(id);
        result.push(child);
        if (result.length >= MAX_DESCENDANT_SESSIONS)
            break;
    }
    return result;
}
function childSessionIDs(children) {
    return children
        .map((child) => isRecord(child) ? toNonEmptyString(child.id) : undefined)
        .filter((id) => Boolean(id));
}
async function getDescendantSessions(api, sessionID, directChildren) {
    ...
    for (let depth = 0; depth < MAX_DESCENDANT_DEPTH && frontier.length > 0; depth += 1) {
        ...
        for (const child of frontier) {
            ...
            current.push(id);
            if (descendants.length >= MAX_DESCENDANT_SESSIONS)
                return descendants;
        }
...
async function getClientSessionMessages(api, sessionID, timeoutMs = SESSION_MESSAGES_FALLBACK_TIMEOUT_MS) {
    const deadline = Date.now() + timeoutMs;
    const attempts = [
        [{ sessionID }],
        [{ path: { sessionID } }],
    ...
    for (const args of attempts) {
        ...
        if (remainingMs <= 0)
            return [];
        try {
            const result = await clientCallWithTimeout((signal) => Promise.resolve(api.client?.session?.messages?.(...args, { signal })), remainingMs);
            if (result.timedOut)
                return [];
            const response = result.value;
            const data = isRecord(response) && "data" in response ? response.data : response;
...
async function sessionMessages(api, sessionID) {
    const local = api.state.session.messages(sessionID);
    if (sessionTokenTotals(local).total > 0)
        return local;
    const status = sessionStatusLabel(api.state.session.status(sessionID));
    if (!allowsBackgroundStatuslineRefresh(status))
        return local;
    const remote = await getClientSessionMessages(api, sessionID);
    if (sessionTokenTotals(remote).total > 0)
        return remote;
    return local.length > 0 ? local : remote;
}
async function sessionMessagesWithChildren(api, messages, children) {
    const childMessages = await mapWithConcurrency(childSessionIDs(children), SESSION_CHILD_CONCURRENCY, (childID) => sessionMessages(api, childID));
    return [...messages, ...childMessages.flat()];
}
async function subagentText(api, sessionID, children) {
...
function quotaText(window, label) {
    if (!window?.usedPercent && window?.usedPercent !== 0)
        return undefined;
    return `${label} ${formatPercent(window.usedPercent)}`;
}
function providerBalanceRank(balance) {
    const label = balance.label.toLowerCase();
    if (label.includes("remaining"))
        return 0;
...
function displayableBalanceValue(balance) {
    const value = toNonEmptyString(balance.value);
    return value && /\d/.test(value) ? value : undefined;
}
export function providerBalanceText(report) {
    if (!report?.ok)
        return undefined;
...
export async function buildTuiStatuslineParts(api, sessionID) {
    ...
    const childDataPromise = childFields && canRefreshBackgroundFields
        ...
        : Promise.resolve({ children: [], allMessages: [...messages] });
...
export async function buildTuiStatusline(api, sessionID) {
