...
for (const field of STATUSLINE_FIELDS) {
    ...
    for (const alias of field.aliases)
        FIELD_BY_TOKEN.set(alias, field.id);
}
export function getStatuslineConfigPath() {
    if (process.env.OPENCODE_STATUSLINE_CONFIG)
        return process.env.OPENCODE_STATUSLINE_CONFIG;
    return path.join(getOpencodeDataDir(), "statusline-plugin.json");
}
export function normalizeStatuslineField(value) {
    return FIELD_BY_TOKEN.get(value.trim().toLowerCase().replace(/[-\s]+/g, "_"));
}
export function uniqueFields(fields) {
    const seen = new Set();
    const result = [];
...
export function loadStatuslineConfig() {
    try {
        const parsed = JSON.parse(fs.readFileSync(getStatuslineConfigPath(), "utf8"));
...
export function saveStatuslineConfig(config) {
    const file = getStatuslineConfigPath();
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, `${JSON.stringify({ version: STATUSLINE_CONFIG_VERSION, fields: uniqueFields(config.fields) }, null, 2)}\n`);
}
export function parseStatuslineFieldArguments(args) {
    const tokens = args
        .split(/[\s,]+/g)
    ...
    for (const token of tokens) {
        ...
        if (field)
            fields.push(field);
        else if (toNonEmptyString(token))
            unknown.push(token);
    }
    return { clear: false, fields: uniqueFields(fields), unknown };
}
//# sourceMappingURL=statusline-config.js.map